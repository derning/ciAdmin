...
//给需要修改的位置添加修给行为
$('span[ectype="inline_edit"]').live('click',function(){
    ...
    $('<input type="text">').css({border:'1px solid #ccc',width:'40%',height:'20px'})
                        ...
                        if(event.keyCode == 13)
                        {
                            ...
                            $.get('?c='+app+'&m=ajax_col&ajax=1',{id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                                if(data === 'false')
                                {
                        ...
                        $(this).prev('span').show().text($(this).attr('value'));
                        $.get('?c='+app+'&m=ml&ajax=1',{id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                            if(data === 'false')
                                {
                                    alert(lang.name_exist);
                                    $('span[fieldname="'+s_name+'"][fieldid="'+s_id+'"]').text(s_value);
...
$('img[ectype="inline_edit"]').click(function(){
                ...
                $.get('?c='+app+'&m=ajax_col&ajax=1',{id:i_id,column:i_name,value:i_val},function(data){
                if(data === 'true')
                    {
    ...
    location.assign(SITE_URL + '/main.php?' + params.join('&'));
    });
});
    //检查提交内容的必须项
